  ...
  - An active Google Cloud Platform (GCP) account with configured GCP IoT Core entries. You can follow GCP IoT core
    documentation on how to configure the Cloud. This should somehow follow the steps below.
      ...
      * Next, in the newly created registry, add a new device:
          ...
          * Add the public key pem content to your device and keep the private key
            which will be passed to the device through console interface.
  ...
  - Stays idle, pending on local user, or cloud-initiated events.
    ...
    - Possible local user actions:
      ...
      - Single push on the user button:
            Trigs a sampling of the sensor values (or simple any application data if no sensor) and their publication 
            to the broker through an MQTT publish message to the "/devices/{device-id}/events" telemetry topic.
