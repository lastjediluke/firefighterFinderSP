  ...
  if (sock->underlying_sock_ctxt >= 0)
  {
    switch(sock->proto)
    {
      ...
      case NET_PROTO_UDP:
        ...
        /* The UDP port may be used both in client mode or in server mode, and the
         * remote host can change over time.
         */
        proto = C2C_UDP_SERVICE_PROTOCOL; /* C2C_UDP_PROTOCOL; */
        rc = NET_OK;
        break;
      default:
        ;
...
int net_sock_recvfrom_udp_c2c(net_sockhnd_t sockhnd, uint8_t * const buf, size_t len, net_ipaddr_t * remoteaddress, int * remoteport)
{
  ...
  { /* Read the received payload by chunks of C2C_PAYLOAD_SIZE bytes because of
     * a constraint of C2C_ReceiveData() - TO BE CONFIRMED. */
    do
    {
      uint16_t port = 0;
      ...
      status = C2C_ReceiveDataFrom((uint8_t) ((uint32_t)sock->underlying_sock_ctxt & 0xFF), tmp_buf, tmp_len, &read,
                               (sock->blocking == true) ? sock->read_timeout : NET_DEFAULT_NOBLOCKING_READ_TIMEOUT, ip , &port);
      msg_debug("Read %d/%d.\n", read, tmp_len);
      if (status != C2C_RET_OK)
      {
        msg_error("net_sock_recv(): error %d in C2C_ReceiveDataFrom() - socket=%d requestedLen=%d received=%d\n",
               status, (int) sock->underlying_sock_ctxt, tmp_len, read);
        rc = NET_EOF;
        break;  
      }
      else
      {
        if (read > tmp_len)
        {
          msg_error("C2C_ReceiveDataFrom() returned a longer payload than requested (%d/%d).\n", read, tmp_len);
          rc = NET_ERR;
          break;
        }
        tmp_buf += read;
...
int net_sock_sendto_udp_c2c(net_sockhnd_t sockhnd, const uint8_t * buf, size_t len, net_ipaddr_t * remoteaddress, int remoteport)
{
  ...
  do
  {
    ...
    status = C2C_SendDataTo((uint8_t) ((uint32_t)sock->underlying_sock_ctxt & 0xFF), (uint8_t *)buf, len, &sent,
